...
trainData = pandas.read_csv('C:\\Users\\galla\\CS-484-Good-1\\Assignment 4\\SpiralWithCluster.csv',
                              usecols = ['id', 'x', 'y'])
...
#print(kNN_Mean)
#
#print(pandas.crosstab(trainData['id'],trainData['_PredictedClass_']))
#
##carray = ['red', 'green']
##plt.figure(dpi=200)
##for i in range(2):
...
#    objTree = tree.DecisionTreeClassifier(criterion='entropy', max_depth=k, random_state=20211111)
#    thisFit = objTree.fit(xTrain, yTrain)
#    tree_accuracy[k-1,0] = k
#    tree_accuracy[k-1,1] = objTree.score(xTrain, yTrain)
#
#
#objTree = tree.DecisionTreeClassifier(criterion='entropy', max_depth=2, random_state=20211111)
#thisFit = objTree.fit(xTrain, yTrain)
#
...
trainData['SpectralCluster'] = y_predictClass

svm_Mean = trainData.groupby('SpectralCluster').mean()
print(svm_Mean)
...
for i in range(2):
    ...
    plt.scatter(x = subData['x'],
                y = subData['y'], c = carray[i], label = i, s = 25)
plt.scatter(x = svm_Mean['x'], y = svm_Mean['y'], c = 'black', marker = '.', s = 100)
